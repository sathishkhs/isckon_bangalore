...
//   }
  ...
  protected function campaignspagewisecontent($page_slug){
    $this->festivals_model->primary_key = array('page_slug' => $page_slug);
    $this->data['page_items'] = $page_items = $this->festivals_model->get_row('campaigns',$this->preview); 

  $this->data['banner'] = $page_items->banner;
//   $this->data['sevas'] = $sevas = $this->festivals_model->get_sevas($page_items->festival_id);
  $template_path = $this->attachtemplatewidgets($page_slug,$page_items);

  return $template_path;
}
  protected function chapterspagewisecontent($page_slug){
    $this->festivals_model->primary_key = array('page_slug' => $page_slug);
    $this->data['page_items'] = $page_items = $this->festivals_model->get_row('chapters',$this->preview); 
...
//   $this->data['sevas'] = $sevas = $this->festivals_model->get_sevas($page_items->festival_id);
  ...
  protected function programpagewisecontent($page_slug){
   
      $this->charitable_programs_model->primary_key = array('page_slug' => $page_slug);
      $this->data['charitable_program'] = $page_items = $this->charitable_programs_model->get_row('charitable_programs',$this->preview); 

    $this->data['banner'] = $page_items->banner;

    $this->data['programs'] = $sevas = $this->charitable_programs_model->get_programs($page_items->charitable_program_id);

    $template_path = $this->attachtemplatewidgets($page_slug,$page_items);
  //   $this->doctors_model->primary_key = array('doctors_id' => $page_items->doctors_id);
  //   $this->data['doctors'] = $this->doctors_model->view_rowdata('doctors');
   
    $this->charitable_programs_model->primary_key = array('charitable_program_id' => $page_items->charitable_program_id);
    // $this->data['testimonials'] = $this->doctors_model->getdata('testimonials');
  ...
  protected function attachtemplatewidgets($page_slug,$page_items){
      ...
      $template_path = $this->templates_model->get_template_path($template_id);    
            ...
            //Gallery page if exist
           
            if(!empty($page_items->category_id)){
                $this->gallery_model->primary_key = array('category_id'=>$page_items->category_id);
                $this->data['categories'] = $this->gallery_model->get_row('category_gallery');
                  $this->gallery_model->primary_key = array('category_id'=>$page_items->category_id);
                  $this->data['galleries'] = $this->gallery_model->view_data('gallery');
                  $this->gallery_model->primary_key = array('gallery_id'=>$page_items->gallery_id);
                  $this->data['gallery_images'] = $this->gallery_model->view_data('gallery_images');
      ...
      $this->page_types_model->primary_key = array('type_id' => !empty($page_items->type_id) ? $page_items->type_id : '');
      $page_type = $this->page_types_model->get_row();
      
      $type_id = !empty($page_type->type_id) ? $page_type->type_id : '';
      $page_type_model = !empty($page_type->model_name) ? $page_type->model_name : '';
      $this->load->model($page_type_model);
      
      $this->data['view_path'] = !empty($page_type->view_path) ? $page_type->view_path : '';
