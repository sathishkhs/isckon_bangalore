...
class Charitable_programs extends MY_Controller
{
	...
	public function index(){
		$data['query'] = $this->charitable_programs_model->view_data('charitable_programs');
...
public function add_charitable_program(){
    ...
    $data['galleries'] = $this->charitable_programs_model->view_data('gallery');
...
public function charitable_program_save()
{
   ...
   // $this->charitable_programs_model->data['seva_category_id'] = implode(',',$this->input->post('seva_category_id'));
    ...
    if (!empty($_FILES['impact_side_image']['name']) && $this->upload->do_upload('impact_side_image')) {
        $upload_data = $this->upload->data();
        $file_Ext =  pathinfo($upload_data['full_path'], PATHINFO_EXTENSION);
        $gen_filename = "impact_side_image" . rand(1000000, 9999999) . "." . $file_Ext;
        rename($upload_data['full_path'], $upload_data['file_path'] . $gen_filename);
        $this->charitable_programs_model->data['impact_side_image'] = $gen_filename;
        // $this->createthumbs(array($upload_data['file_name']));
    } else {
    
        $data['form_error']['impact_side_image'] = $this->upload->display_errors();
    }
    if (!empty($_FILES['explore_side_image']['name']) && $this->upload->do_upload('explore_side_image')) {
        $upload_data = $this->upload->data();
        $file_Ext =  pathinfo($upload_data['full_path'], PATHINFO_EXTENSION);
        $gen_filename = "explore_side_image" . rand(1000000, 9999999) . "." . $file_Ext;
        rename($upload_data['full_path'], $upload_data['file_path'] . $gen_filename);
        $this->charitable_programs_model->data['explore_side_image'] = $gen_filename;
        // $this->createthumbs(array($upload_data['file_name']));
    } else {
    
        $data['form_error']['explore_side_image'] = $this->upload->display_errors();
    }
   
     $this->program_video = array('upload_path' => CHARITABLE_PROGRAM_BANNER_PATH, 'allowed_types' => 'mp4|avi|mpg|mpeg|wmv|mov');
    $this->upload->initialize($this->program_video);
   
    if (!empty($_FILES['program_video']['name']) && $this->upload->do_upload('program_video')) {
        $upload_data = $this->upload->data();
        $file_Ext =  pathinfo($upload_data['full_path'], PATHINFO_EXTENSION);
        $gen_filename = "program_video" . rand(1000000, 9999999) . "." . $file_Ext;
        rename($upload_data['full_path'], $upload_data['file_path'] . $gen_filename);
        $this->charitable_programs_model->data['program_video'] = $gen_filename;
        // $this->createthumbs(array($upload_data['file_name']));
    } else {
    
        $data['form_error']['program_video'] = $this->upload->display_errors();
    }
    ...
    if (!empty($charitable_program_id)) {
        ...
        $this->charitable_programs_model->primary_key = array('charitable_program_id' => $charitable_program_id);
        if ($r = $this->charitable_programs_model->update('charitable_programs')) {
