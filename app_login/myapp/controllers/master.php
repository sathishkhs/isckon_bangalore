...
class Master extends MY_Controller
{
	...
	public function banner_save()
	{
		...
		// print_r($this->master_model->data);exit;
		...
		if (!empty($banner_id)) {
			...
			if ($q = $this->master_model->insert('banners')) {
	...
	public function removeimg($banner_id)
	{
		...
		if (!empty($banner_id)) {
			$this->master_model->primary_key = array('banner_id' => $banner_id);
			if ($this->master_model->update_image('banners')) {
				// unlink(BANNERS_PHOTO_UPLOAD_PATH . $banner_background_img_path);
				$msg = array('type' => 'success', 'icon' => 'lni lni-thumbs-up lni-lg mr-2', 'txt' => 'Image deleted Successfully');
		} else {
			$msg = array('type' => 'danger', 'icon' => 'lni lni-thumbs-down lni-lg mr-2', 'txt' => 'Sorry! Unable to delete.');
		}
		$this->session->set_flashdata('msg', $msg);
		redirect('master/banner_edit/'.$banner_id);
		}
	}
	public function banner_list()
	{
		...
		foreach ($banners->result() as $row) {
			...
			<button class="btn btn-primary btn-small btn-xs"><i class="fa fa-edit"></i></button></a>&nbsp;
			<a class="red" title="Delete" href="master/banner_delete/' . $row->banner_id . '/'. $row->banner_background_img_path. '"> 
			<button class="btn btn-danger btn-small btn-xs"><i class="fa fa-trash"></i></button></a>&nbsp;
			</div></td>
			');
			}
	...
	public function pages_add()
	{
		...
		$data['galleries'] = $this->master_model->view('gallery');
		$data['video_galleries'] = $this->master_model->view('video_gallery');
		...
		$data['scripts'] = array('../js-plugins/summernote/summernote-lite.js','assets/javascripts/master.js');
		$this->load->view('templates/dashboard', $data);
	}
...
// 		$data['forms'] = $this->master_model->view('form');
	...
	public function pages_save()
	{
		...
		$this->video_cover_img_1 = array('upload_path' => VIDEO_COVER_IMAGE_PATH, 'allowed_types' => 'gif|jpg|png|jpeg');
		$this->upload->initialize($this->video_cover_img_1);
		if (!empty($_FILES['video_cover_img_1']['name']) && $this->upload->do_upload('video_cover_img_1')) {
			$upload_data = $this->upload->data();
			$file_Ext =  pathinfo($upload_data['full_path'], PATHINFO_EXTENSION);
			$gen_filename = "video_cover_img_1_" . rand(1000000, 9999999) . "." . $file_Ext;
			rename($upload_data['full_path'], $upload_data['file_path'] . $gen_filename);
			$this->master_model->data['video_cover_img_1'] = $gen_filename;
			// $this->createthumbs(array($upload_data['file_name']));
		} else {
			$data['form_error']['video_cover_img_1'] = $this->upload->display_errors();
		}
		$this->video_cover_img_2 = array('upload_path' => VIDEO_COVER_IMAGE_PATH, 'allowed_types' => 'gif|jpg|png|jpeg');
		$this->upload->initialize($this->video_cover_img_2);
		if (!empty($_FILES['video_cover_img_2']['name']) && $this->upload->do_upload('video_cover_img_2')) {
			$upload_data = $this->upload->data();
			$file_Ext =  pathinfo($upload_data['full_path'], PATHINFO_EXTENSION);
			$gen_filename = "video_cover_img_2_" . rand(1000000, 9999999) . "." . $file_Ext;
			rename($upload_data['full_path'], $upload_data['file_path'] . $gen_filename);
			$this->master_model->data['video_cover_img_2'] = $gen_filename;
			// $this->createthumbs(array($upload_data['file_name']));
		} else {
			$data['form_error']['video_cover_img_2'] = $this->upload->display_errors();
		}
		
		$this->video_cover_img_3 = array('upload_path' => VIDEO_COVER_IMAGE_PATH, 'allowed_types' => 'gif|jpg|png|jpeg');
		$this->upload->initialize($this->video_cover_img_3);
		if (!empty($_FILES['video_cover_img_3']['name']) && $this->upload->do_upload('video_cover_img_3')) {
			$upload_data = $this->upload->data();
			$file_Ext =  pathinfo($upload_data['full_path'], PATHINFO_EXTENSION);
			$gen_filename = "video_cover_img_3_" . rand(1000000, 9999999) . "." . $file_Ext;
			rename($upload_data['full_path'], $upload_data['file_path'] . $gen_filename);
			$this->master_model->data['video_cover_img_3'] = $gen_filename;
			// $this->createthumbs(array($upload_data['file_name']));
		} else {
			$data['form_error']['video_cover_img_3'] = $this->upload->display_errors();
		}
	...
	public function pages_list(){
		...
		foreach ($pages->result() as $row) {
			...
			// $modified_by = ($row->last_modified_by == 1) ? $row->created_by : $row->last_modified_by;
			$this->admin_users_model->primary_key = array('user_id'=>$this->session->userdata('user_id'));
			$modified_by = $this->admin_users_model->get_row()->username;
			
			$this->master_model->primary_key = array('type_id'=>$row->type_id);
			$type_name = $this->master_model->get_row('page_types');
			$status = (!empty($row->status_ind) && ($row->status_ind == 1)) ? "<i class='fa fa-check-circle text-success'>Active</i> &nbsp;&nbsp;" : "<i class='nav-icon fa  fa-times-circle text-danger' >Inactive</i>";
			$date =  ($row->last_modified_date == "") ?  $row->created_date : $row->last_modified_date;
			$data[] = array(
				$row->page_title,
				!empty($type_name->type_name) ? $type_name->type_name : '',
				$row->page_slug,
				$modified_by,
				$date,
				$status,
	...
	public function settings()
	{
		...
		$data['scripts'] = array('../js-plugins/summernote/summernote-lite.min.js');
		$this->load->view('templates/dashboard', $data);
	}
	...
	public function menuitem_save()
	{
		...
		if (empty($menuitem_id)) { //add Case
			...
			$this->master_model->data['modified_by']	= $this->session->userdata('user_id');
			$this->master_model->data['status_ind']	= $this->input->post('status_ind');
			if ($res = $this->master_model->insert('menuitems')) {
